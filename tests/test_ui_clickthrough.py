...
def test_ui_clickthrough_full_operator_flow(qtbot, settings_store, tmp_path: Path, monkeypatch) -> None:
    ...
    assert window.job_state.img_move_x_mm == 123
    assert window.job_state.img_move_y_mm == 77
    assert window.txt_move_x.text() == "123"
    assert window.txt_move_y.text() == "77"
    assert int(window.txt_center_x.text()) > 123
    assert int(window.txt_center_y.text()) > 77
    ...
    assert int(window.txt_center_x.text()) > 0
    assert int(window.txt_center_y.text()) > 0
    ...
    assert window.txt_center_x.text() == "0"
    assert window.txt_center_y.text() == "0"
    assert warnings == []
